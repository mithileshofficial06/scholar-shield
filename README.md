...
_Generated by `npm run metrics` on 2026-09-18. Never hand-edited._

| Metric | Value |
|---|---|
| Precision@10 | 100.0% (base rate 53.5%) |
| Queue lift (top decile vs. base rate) | 1.87× (top 4 of 43) |
| Holdout recall | 38.2% (13/34) |
...
**The sealed holdout is the honest accuracy number, and it is 38.2%.** Known recall of 100.0% measures only that the rules fire on the patterns they were written against — it is a consistency check, not evidence. Against the patterns sealed before any rule code existed, the engine surfaced 13 of 34 applications it should have.

Unrecovered patterns: `HO-02-threshold-bunching`, `HO-04-certificate-serial-adjacency`, `HO-09-identical-round-income-mill`, `HO-10-deliberate-household-split`. Each needs a rule the current set does not contain — income bunching below the ceiling, certificate serial adjacency, reuse of one certificate across applicants, family-size inflation, shared contact details across nominally distinct households, and deliberate household splitting.
